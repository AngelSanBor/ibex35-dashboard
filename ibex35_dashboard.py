...
.dash-sub {
    ...
    font-size: 0.75rem; color: #555; letter-spacing: 2px;
...
/* Position banner */
.pos-banner {
    background: linear-gradient(135deg, #0f1f17 0%, #0a0f1a 100%);
...
.pos-label {
    font-family: 'DM Mono', monospace;
    font-size: 0.65rem; color: #3a7a5a;
    text-transform: uppercase; letter-spacing: 2px;
}
.pos-value {
    font-family: 'DM Mono', monospace;
    font-size: 1rem; color: #00e5a0; font-weight: 500;
}
...
.monitor-price {
    font-family: 'DM Mono', monospace;
    font-size: 1.1rem; color: #e8e8e8; font-weight: 500;
}
...
with st.sidebar:
    ...
    intervalo_map = {"1d":"5m","5d":"15m","1mo":"1d","3mo":"1d","6mo":"1d","1y":"1wk","2y":"1wk"}
    intervalo = intervalo_map[periodo]
...
<div class="dash-header">
    ...
    <span class="dash-sub">datos en tiempo real · Yahoo Finance</span>
</div>
""", unsafe_allow_html=True)

# ── Carga de datos ────────────────────────────────────────────────────────────
@st.cache_data(ttl=300)
def cargar_datos(ticker, periodo, intervalo):
...
df = cargar_datos(ticker, periodo, intervalo)

if df is None or df.empty:
    st.error(f"No se pudieron cargar datos para {ticker_nombre}.")
    st.stop()
...
<div class="metric-grid">
    <div class="metric-card">
        ...
        <div class="metric-value">{ticker_nombre}</div>
    </div>
    <div class="metric-card">
    ...
    <div class="metric-card">
        <div class="metric-label">Variación ({periodo})</div>
        <div class="metric-value {chg_class}">{chg_sign} {abs(cambio_pct):.2f}%</div>
    </div>
    <div class="metric-card">
        <div class="metric-label">Máximo ({periodo})</div>
        <div class="metric-value">{maximo:.2f} €</div>
    </div>
    <div class="metric-card">
        <div class="metric-label">Mínimo ({periodo})</div>
        <div class="metric-value">{minimo:.2f} €</div>
    </div>
    <div class="metric-card">
        <div class="metric-label">Vol. medio</div>
        <div class="metric-value">{volumen_medio:,}</div>
    </div>
</div>
""", unsafe_allow_html=True)
...
if acciones_input > 0 and entrada_input > 0:
    ...
    <div class="pos-banner">
        <div>
            <div class="pos-label">Inversión</div>
            <div class="pos-value">{inversion:,.0f} €</div>
        </div>
        <div>
            <div class="pos-label">Valor actual</div>
            <div class="pos-value">{valor_actual:,.0f} €</div>
        </div>
        <div>
            <div class="pos-label">P&L €</div>
            <div class="{pos_class}">{signo} {abs(pl_eur):,.0f} €</div>
        </div>
        <div>
            <div class="pos-label">P&L %</div>
            <div class="{pos_class}">{signo} {abs(pl_pct):.2f}%</div>
        </div>
        <div>
            <div class="pos-label">Acciones</div>
            <div class="pos-value">{acciones_input:,}</div>
        </div>
        <div>
            <div class="pos-label">Entrada</div>
            <div class="pos-value">{entrada_input:.2f} €</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
# Línea de entrada personalizada
if acciones_input > 0 and entrada_input > 0:
...
if mostrar_rsi and "RSI14" in df.columns:
    ...
    fig.update_yaxes(title_text="RSI", range=[0,100], row=fila_actual, col=1,
                     title_font_color="#555", tickfont_color="#555")
    fila_actual += 1
...
if mostrar_macd and "MACD" in df.columns:
    ...
    fig.add_trace(go.Bar(x=df.index, y=df["Hist"],
        name="Histograma", marker_color=colores, opacity=0.6), row=fila_actual, col=1)
    fig.update_yaxes(title_text="MACD", row=fila_actual, col=1,
                     title_font_color="#555", tickfont_color="#555")
...
fig.update_layout(
    ...
    legend=dict(orientation="h", y=1.02, x=0,
                bgcolor="rgba(0,0,0,0)", font=dict(color="#555", size=11)),
...
fig.update_xaxes(showgrid=True, gridcolor="#1a1a24", zeroline=False,
                 tickfont=dict(color="#444"), linecolor="#1a1a24")
...
def precios_ibex(tickers_dict):
    ...
    for nombre, t in tickers_dict.items():
        try:
            d = yf.download(t, period="2d", interval="1d", auto_adjust=True, progress=False)
...
with st.expander("📋 Datos históricos"):
    df_mostrar = df[["Open","High","Low","Close","Volume"]].copy()
    df_mostrar.columns = ["Apertura","Máximo","Mínimo","Cierre","Volumen"]
